		...
		<>
			...
			<form className="mr-3 m-0 md:m-5 flex flex-wrap">
				<div className="w-[100%] md:w-[50%] xl:w-[30%] h-90  md:mr-5">
					<ImageGroupComponent readonly value={product.data.images.map((x) => x.url)} />
				</div>
				<div className="w-[100%] md:w-[40%] xl:w-[65%] md:mt-5">
					<InputComponent label={"Product Name"} value={product.data.name} />
					<InputComponent label={"Product Small Description"} value={product.data.small_description} />

					<div className="flex w-full gap-2">
						<InputComponent readonly label={"Price"} value={product.data.price} />
						<InputComponent label={"Discount"} readonly value={product.data.discount} />
					</div>
				</div>
				<div className="w-[100%] md:w-[100%] xl:w-[100%] md:my-5">
					...
					<InputComponent textarea label={"Description"} value={product.data.description} />
...
const ImageGroupComponent = ({ register, error, value, readonly = false }) => {
	return (
		<div className="grid grid-cols-3 grid-rows-3 w-[100%] h-[100%] gap-3 m-1 relative">
			<div className="">
				<ImageComponent value={value[1]} register={register} readonly={readonly} name={"images[1]"} registerObj={{ required: "Image is required" }} />
			</div>
			<div className="row-span-3 col-span-2">
				<ImageComponent readonly={readonly} value={value[0]} register={register} name={"images[0]"} registerObj={{ required: "Main Image is required" }} />
			</div>
			<div className=" ">
				<ImageComponent readonly={readonly} value={value[2]} register={register} name={"images[2]"} />
			</div>
			<div className="">
				<ImageComponent readonly={readonly} value={value[3]} register={register} name={"images[3]"} />
			</div>
			<h2 className="absolute -bottom-5 text-red-500">{error && error.filter((x) => x)[0]?.message}</h2>
		</div>
	);
};

const ImageComponent = ({ name = "", register = () => ({}), registerObj, value = null, readonly }) => {
	...
	const { ref: registerRef = () => {}, onChange: registerOnChange = () => {}, ...registerRest } = register(name, registerObj);
		...
		>
			{value ? (
				<div className="pl-3 lg:pl-10 ">
					...
					<div className="flex items-center   gap-3">
						Size :{" "}
						<span className={`font-bold text-[${value.color}]`} style={{ color: value.color }}>
							{value.size}
						</span>
					</div>
					<div className="flex items-center   gap-3">
