...
import { Package2, Package2Icon, PackageCheck, PackageMinus, PackageOpen } from "lucide-react";
		...
		},
		{
			...
			render: (val, row) => (
				<div className="flex w-full text-nowrap items-center gap-3 px-3">
					<img src={row?.images[0]?.url} className="rounded-full w-10 h-10" alt="" />
					{val}
				</div>
			),
		},
		...
		// },
		{
			...
			render: (val, row) => (
				<div className="flex gap-5">
					...
					<Link className="bg-green-500 hover:bg-green-600 text-white py-1 px-4 rounded font-semibold text-xs" to={"edit/" + row._id}>
						Edit
					</Link>
					<button
