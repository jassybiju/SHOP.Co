...
import AddressComponent from "@/app/User/Accounts/pages/address/component/AddressComponent";
...
const ViewOrderManagement = () => {
	...
	const { mutate: updateStatus, status: mutatingStatus } = useUpdateOrderStatus();
	const { mutate: returnOrder, status: returningStatus } = useReturnOrderItemStatus();
			...
			<div className="w-full gap-2 flex p-3">
				<div className="w-3/5 gap-2 flex flex-col">
					...
					<div className="w-full rounded-2xl border-2 bg-white ">
						...
						<div className=" flex flex-col sm:flex-row p-4 sm:justify-between gap-4 sm:gap-0 border-b-2">
							<div className="flex flex-col sm:flex-row sm:gap-8 flex-1">Customer Information</div>
						</div>
						<div className="p-4">
									...
									>
										{x.is_cancelled || x.is_returned && (
											<div className="absolute rounded-2xl flex items-center justify-center w-full h-full bg-gray-400/60 text-center text-3xl">
												<p className="-rotate-22 ">{x.is_cancelled ? "Cancelled" : "Returned"}</p>
											</div>
										)}
										...
										<div className="flex h-full gap-2">
											...
											<div className="flex flex-col justify-evenly h-full">
                                                ...
                                                {x.status}
												{x.status === 'RETURN_REQUESTED' && <div className="flex w-full gap-4">
													<Button
														onClick={() =>
														...
														className={"bg-green-400 h-1/2  text-nowrap  w-full hover:bg-green-700"}
													...
													<Button
														...
														className={"py-2  h-1/2 bg-red-400 w-full hover:bg-red-700 "}
														label="Return Denied"
													/>
												</div>
                                                    }
				...
				<div className="w-2/4 flex flex-col gap-2 ">
					...
					<div className="w-full rounded-2xl border-2 bg-white ">
						{/* ! HEADER  */}
						<div className=" flex flex-col sm:flex-row p-4 sm:justify-between gap-4 sm:gap-0 border-b-2">
							<div className="flex flex-col sm:flex-row sm:gap-8 flex-1">Update Order Status</div>
						</div>
						<div className="p-4">
							<InputComponent
								select
								...
								options={STATUS_TRANSITIONS_OPTIONS[status_history.slice(-1)[0].status]}
							/>
							<Button
								onClick={() =>
									onClickComment((x) => {
										updateStatus({ id, data: { status: selectedStatus, description: x } });
									}, "update status")
								}
					...
					<div className="w-full rounded-2xl border-2 bg-white  ">
						...
						<div className="p-4 flex flex-col gap-3">
							<div className="w-full flex justify-between">
								<span className="">
									Subtotal <span className="text-gray-500 text-sm">( {items.length} item(s) )</span>:{" "}
								</span>
