...
const InputComponent = ({
	...
	let input;
	if (colorPicker) {
		input = <input type="color" defaultValue={defaultValue || "#ffffff"} ref={handleRef} disabled={readonly} {...registerRest} {...rest} />;
	} else if (select) {
		input = (
			<select
				id={id}
	...
	return (
		<div className={`mb-7 w-full max-w-full`} style={{ width: `${width}%` }}>
			...
			<div className="relative">
				{input}
				{errors && <p className="text-red-500 text-xs mt-2 absolute left-0">{errors.message}</p>}
			</div>
		</div>
	);
};
